-- Finance V1.1 Stage 4D - Transaction Trash Mutation.
--
-- Canonical idempotent mutation to transition ACTIVE Expense/Income to TRASHED,
-- and if an associated Account effect exists, transition it ACTIVE -> REVERSED.
-- Atomic, idempotent, preserves history. No frontend. No Restore.

create extension if not exists "pgcrypto";

-- Canonical Trash mutation RPC
create or replace function public.finance_trash_transaction_v1(
...
declare
  ...
  v_scope_type text;
  v_scope_id uuid;
  v_operation text := 'finance.transaction.trash';
...
begin
  ...
  -- Authorization: verify caller can trash this transaction
  ...
  -- Commission Provenance Guard (Stage 4D):
  -- A commission-generated Expense (transfer_id IS NOT NULL) cannot be
  -- independently Trashed. It belongs to the owning Transfer.
  ...
  if v_transaction.status = 'TRASHED' then
    -- Idempotent: already TRASHED, return current state
    update public.planner_idempotency_keys
  ...
  if v_transaction.status <> 'ACTIVE' then
    raise exception 'invalid_transaction_state_for_trash' using errcode = '23514';
  end if;
  ...
  -- 1. Transition transaction: ACTIVE -> TRASHED
  ...
  -- 2. If effect exists, transition: ACTIVE -> REVERSED
...
exception
  when sqlstate '00000' then
    ...
    if SQLERRM = 'replay' then
      ...
      -- This is a special case for replay; we need to return the stored transaction
      -- Fetch directly from finance_transactions using the ID stored in response_body
      select *
        into v_transaction
      from public.finance_transactions
...
grant execute on function public.finance_trash_transaction_v1(uuid, text, text, text, uuid) to authenticated;
