...
* Calendar real construido desde eventos. En el MVP actual no se mezclan tareas con fecha dentro de Calendar.  
...
* Responsabilidades iniciales:  
  ...
  * Finanzas;  
  * Otro.  
  * Nota: usar como fuente real las responsabilidades ya creadas/existentes en el proyecto.
...
* Asignación a miembros activos del hogar usando `household_members.id`.  
    
* Refetch automático obligatorio para demo multi-dispositivo; pull-to-refresh recomendado si da el tiempo; realtime verdadero deseable pero no bloqueante.  
...
* No enviar `household_id` desde frontend.
...
* No usar `household_id` manual desde frontend.  
...
El color principal de HomePlus se mantiene terracota/salvia. Planner debe verse más premium y alineado al Figma sin reemplazar la identidad visual natural de la app.

Uso recomendado:

* Terracota/salvia para:  
...
* No usar el color primario en todos lados.  
    
* Máximo 1 o 2 elementos principales de color primario por pantalla.  
...
* Borde lateral terracota/salvia o warning según el caso.  
* Texto corto.  
* CTA claro.
...
* Mostrar `Tareas`, `Calendario` y `Metas`.  
* `Metas` debe aparecer como `Próximamente`, sin backend real.
...
Planner abre por defecto en la tab `Tareas`.

No se crea una pantalla Overview separada para MVP, porque Home ya cumple el rol de resumen general del hogar.

### Justificación

* Tasks es la acción más frecuente del Planner.  
* El problema principal actual está en tareas.  
* Facilita la demo multi-dispositivo.  
* Reduce complejidad de navegación.  
* Home ya muestra el resumen de tareas/eventos.  
* Planner debe administrar, no duplicar el dashboard de Home.
...
2. Tabs internas:  
   * `Tareas`;  
   * `Calendario`;  
   * `Metas` como `Próximamente`.  
3. Mini resumen arriba de Tareas:  
   * pendientes;  
   * hoy;  
   * vencidas;  
   * por verificar.  
...
No implementar Overview como pantalla separada en el MVP actual.
...
* `overdue`, si no completada/cancelada/verificada y `due_date` ya pasó.
...
* completar → `awaiting_verification`.  
* verificar → `verified`.

Si backend modela verificación con campos y no como status, el frontend puede derivar visualmente:

* `awaiting_verification` \= completada \+ requiere verificación \+ sin `verified_at`.  
* `verified` \= completada \+ requiere verificación \+ con `verified_at`.
...
* Refetch automático al entrar o volver a Planner.  
* Pull-to-refresh recomendado si da el tiempo, pero no bloqueante.  
* Skeleton al cargar.
...
Debe usar `household_members.id` como valor real.
...
* `assigned_to_member_id` debe ser de miembro activo.  
* No mandar `household_id`.  
* No mandar `created_by_person_id`.  
* No mandar `completed_by_person_id`.  
* No mandar `verified_by_person_id`.
...
* Las tareas hechas no aparecen en pendientes.  
* Se consultan desde chip/pestaña `Hechas`.  
...
4. Si éxito:  
   ...
   * estado visual `awaiting_verification`;  
   * badge “Por verificar”.
...
Nota MVP: el catálogo final debe usar las responsabilidades ya creadas/existentes en el proyecto. Esta lista funciona como referencia; no se deben inventar responsabilidades nuevas si el proyecto ya tiene un catálogo definido.
...
* `template_key` constante.  
* No usar tabla.  
* No usar CRUD.  
* No usar endpoints de templates.  
* El frontend manda `template_key` si el backend actual lo soporta.  
...
* Si se elige template/responsabilidad, se completa automáticamente `template_key` y `category`.  
...
* No mandar `completed_by_person_id`.  
* No mandar `verified_by_person_id`.  
* `assigned_to_member_id` debe ser `household_members.id`.  
* Usar `/api/planner`.  
* Usar Bearer token.  
* Usar `planner_tasks`.
...
* Dot terracota/salvia para evento.  
...
* No mostrar tareas con fecha dentro de Calendar en el MVP actual.  
* Calendar debe mostrar fechas y eventos asociados a esas fechas.

Regla:

* Calendar administra eventos.  
* Las tareas con fecha se gestionan desde Tasks y pueden aparecer en Home, pero no se mezclan en Calendar durante el MVP actual.
...
* `title` requerido.  
* `starts_at` requerido.  
* Si `all_day=false`, hora inicio recomendada/requerida según backend.  
* `ends_at` opcional.  
* Si `ends_at` existe, no debe ser anterior a `starts_at`.  
* No mandar `household_id`.  
* Backend resuelve hogar activo.  
* Backend resuelve `created_by_person_id`.
...
Goals/Metas aparece en varios fragments como parte conceptual de Planner, pero no debe implementarse como funcionalidad real en el MVP actual.

### Decisión final MVP

Mostrar `Metas` como tab o card `Próximamente`, sin backend real.

### Reglas

* No permitir crear goal real.  
* No llamar endpoints reales de Goals.  
* No crear services reales de Goals.  
* No mezclar Goals con tareas reales.  
* No bloquear Tasks/Calendar por Goals.  
* Usar copy claro:  
  * “Metas familiares — Próximamente”  
  * “Próximamente vas a poder organizar objetivos del hogar.”

### Justificación

* El MVP actual necesita cerrar Tasks \+ Events \+ Calendar \+ Home conectado.  
* Goals requiere modelo propio.  
* Mostrarlo como `Próximamente` ayuda a que la app se vea más completa sin expandir alcance.
...
### Refetch automático y pull-to-refresh

Debe existir como mínimo:

* Refetch automático al entrar o volver a Home.  
* Refetch automático al entrar o volver a Planner.  
* Refetch automático después de crear, editar, completar, verificar o cancelar tareas/eventos.

Pull-to-refresh queda recomendado para Tasks, Calendar y Home si da el tiempo, pero no debe bloquear el MVP.
...
* Refetch automático después de crear, editar, completar, verificar o cancelar tareas/eventos.  
* Pull-to-refresh recomendado si da el tiempo.  
* Refetch al tocar tab activo.  
* Si existe realtime, actualizar lista al recibir eventos.  
* Si no existe realtime, el refetch automático debe ser confiable para demo.
...
* Usar `planner_events`.  
* No enviar `household_id` desde frontend.  
* Backend resuelve `active_household`.  
* `assigned_to_member_id` debe ser `household_members.id`.  
* `created_by_person_id` lo resuelve backend.  
* `completed_by_person_id` lo resuelve backend.  
* `verified_by_person_id` lo resuelve backend.  
...
* normalizar eventos para la vista Mes \+ Agenda.

No debe ser dueño de datos. Para el MVP actual, `plannerCalendar` no mezcla tareas con fecha dentro del calendario.
...
`assigned_to_member_id` debe ser `household_members.id`, no `people.id`, no `auth.users.id`.
...
5. Planner usa `planner_tasks` y `planner_events` vía backend.  
6. Frontend no envía `household_id`.  
7. Backend resuelve `active_household`.  
8. `assigned_to_member_id` debe ser `household_members.id`.  
...
26. Calendar no muestra tareas con fecha en el MVP actual; muestra fechas y eventos.  
27. Events requieren `starts_at`.  
28. `ends_at` es opcional.  
...
38. Refetch automático debe existir para demo; pull-to-refresh queda recomendado pero no bloqueante.  
...
47. Otro usuario debe poder ver cambios mediante refetch automático, pull-to-refresh si existe o realtime simple.  
...
* Mostrar Goals/Metas como `Próximamente`.  
* No crear backend ni services reales de Goals.  
* No bloquear Tasks/Calendar por Goals.

Requiere validación usuario:

* No. El usuario confirmó que Metas puede aparecer como `Próximamente`, sin backend real.

---

### 2\. Design System original terracota/salvia vs propuesta previa de violeta

Contradicción:

* Design System habla de paleta tierra cálida, primario terracota/salvia.  
* Una versión previa de esta spec proponía violeta como color operativo del Planner.

Resolución MVP:

* Mantener terracota/salvia como color principal de HomePlus.  
* Planner debe verse premium usando la identidad visual existente, no reemplazándola por violeta.  
* Usar terracota/salvia para CTA principal, tab activo, chips seleccionados e indicadores suaves.  
* Usar colores semánticos para success/warning/error.

Requiere validación usuario:

* No. El usuario confirmó que el color natural de la app es terracota/salvia.
...
* No como bloqueo de producto: usar las responsabilidades ya creadas/existentes en el proyecto.
...
* Enviar `template_key` solo si backend actual lo soporta.  
...
* Algunos fragments mencionan `pending`, `in_progress`, `completed`, `cancelled`.  
* El MVP actual pide `pending`, `completed`, `awaiting_verification`, `verified`.  
...
* No mostrar `in_progress` salvo que backend ya lo use y sea necesario.
...
* El MVP actual pide `awaiting_verification` y `verified`.

Resolución MVP:

* UI debe mostrar esos estados.  
* Service puede derivarlos si backend usa campos como `requires_verification` / `verified_at`.  
...
* No. El usuario confirmó Calendar default `Mes + Agenda`.
...
* No. El usuario confirmó Home con cards reales de tareas/eventos y máximo recomendado de 5 tareas.

---

### 9\. API fragment habla de `household_id` explícito vs regla técnica actual dice no enviarlo

Contradicción:

* Fragments antiguos/API pueden hablar de operar con `:hid` o `household_id`.  
* Regla técnica actual: frontend no manda `household_id`; backend resuelve active household.
...
## 29\. Decisiones finales del usuario

Estas decisiones reemplazan las preguntas abiertas anteriores y deben tratarse como fuente de verdad para el prompt de implementación posterior.

* Planner abre por defecto en la tab `Tareas`.  
* No se crea una pantalla Overview separada para MVP; Home ya cumple rol de resumen.  
* La tab/card `Metas` puede aparecer como `Próximamente`, sin backend real.  
* Crear tarea se hace mediante bottom sheet grande, casi pantalla completa, pero sin ocupar toda la pantalla.  
* Crear evento usa el mismo patrón: bottom sheet grande.  
* Las responsabilidades iniciales serán las ya creadas/existentes en el proyecto.  
* Se permite `Otro` como categoría libre para tareas no cubiertas por responsabilidades existentes.  
* El tab `Más/More` puede mostrar módulos mock cuando no exista backend real.  
* Las tareas completadas se ocultan de pendientes y quedan disponibles en filtro/pestaña `Hechas`.  
* Home muestra tareas reales de Planner; máximo recomendado: 5 tareas.  
* Calendar abre por defecto en vista `Mes + Agenda`.  
* Calendar muestra fechas y eventos asociados a esas fechas.  
* Calendar no necesita mostrar tareas con fecha en el MVP actual.  
* Prioridad de tarea: `Baja`, `Normal`, `Alta`.  
* Responsable visible en tareas; asignación opcional para no bloquear creación.  
* La opción de verificación debe estar visible para todas las tareas.  
* El color principal de HomePlus se mantiene terracota/salvia; no se reemplaza por violeta.  
* Quick Actions muestra acciones reales de Planner y acciones mock para módulos no implementados.  
* Tareas canceladas se ocultan por defecto, pero existe filtro para verlas.  
* Eventos cancelados se ocultan por defecto.  
* Home tendrá cards reales de tareas/eventos y cards mock para todo lo que no tenga backend real.  
* Para demo multi-dispositivo, debe implementarse como mínimo refetch automático al entrar o volver a Home/Planner, y después de crear, editar, completar, verificar o cancelar tareas/eventos. Pull-to-refresh queda recomendado si da el tiempo, pero no debe bloquear el MVP. Realtime verdadero es deseable, pero no obligatorio para esta entrega.  
* `template_key` se usa solo si el backend actual lo soporta; si no, queda como metadata interna del frontend para autocompletar título/categoría.

### Decisiones abiertas restantes

No quedan decisiones de producto bloqueantes para esta spec. Antes de Codex solo falta confirmar contratos técnicos reales del backend actual: endpoints, nombres de campos y status exactos.
...
* Tareas/eventos se actualizan con refetch automático al volver a Home/Planner y después de mutaciones.
...
* No manda `household_id`.  
* Backend resuelve active household.  
* `assigned_to_member_id` usa `household_members.id`.  
* Templates son constantes.  
* Goals reales no se implementan.  
* Services están separados y claros.  
* Errores están manejados.  
* Multi-dispositivo demo funciona con refetch automático, pull-to-refresh si existe o realtime.
...
- [x] Planner abre por defecto en `Tareas`.  
- [x] No hay pantalla Overview separada en MVP.  
- [x] `Metas` aparece como `Próximamente`, sin backend real.  
- [x] Crear tarea usa bottom sheet grande/casi full screen.  
- [x] Crear evento usa bottom sheet grande/casi full screen.  
- [x] Responsabilidades iniciales: usar las ya creadas/existentes.  
- [x] Se permite `Otro` como categoría libre.  
- [x] Tareas completadas se ocultan de pendientes y van a `Hechas`.  
- [x] Home muestra máximo 5 tareas reales.  
- [x] Calendar default: `Mes + Agenda`.  
- [x] Calendar muestra fechas y eventos, no tareas con fecha.  
- [x] Prioridad: `Baja`, `Normal`, `Alta`.  
- [x] Responsable visible y asignación opcional.  
- [x] Verificación visible para todas las tareas.  
- [x] Color principal: terracota/salvia, no violeta.  
- [x] Quick Actions muestra acciones reales y mocks del resto.  
- [x] Tareas canceladas ocultas por defecto con filtro visible.  
- [x] Eventos cancelados ocultos por defecto.  
- [x] Home usa cards reales de tareas/eventos y mocks para lo demás.  
- [x] Refetch automático obligatorio para demo; pull-to-refresh recomendado; realtime no bloqueante.  
- [x] `template_key` solo si backend lo soporta; si no, metadata interna del frontend.
...
- [ ] Confirmar shape de `planner_tasks`.  
- [ ] Confirmar shape de `planner_events`.  
...
- [ ] Confirmar si `template_key` existe en backend o queda solo frontend.  
- [ ] Confirmar que `assigned_to_member_id` es `household_members.id`.  
- [ ] Confirmar que frontend no manda `household_id`.
...
- [ ] Definir puntos de refetch automático al volver a Home/Planner y tras mutaciones.

### QA demo

- [ ] Usuario A crea tarea.  
- [ ] Usuario B ve tarea tras refetch automático o refresh manual si existe.  
- [ ] Usuario B completa tarea.  
- [ ] Usuario A ve completada tras refetch automático o refresh manual si existe.  
- [ ] Usuario A crea evento.  
- [ ] Usuario B ve evento tras refetch automático o refresh manual si existe.  
...
\#\# Lógica funcional a contemplar para implementación

La spec frontend no solo define pantallas. También define reglas funcionales que deben respetarse en la implementación.

\#\#\# Lógica frontend obligatoria

\- Planner abre por defecto en \`Tareas\`.  
\- No hay Overview separado en MVP.  
\- \`Metas\` se muestra como \`Próximamente\`, sin backend real.  
\- Crear tarea y crear evento usan bottom sheet grande.  
\- Las tareas completadas se ocultan de pendientes y aparecen en filtro/pestaña \`Hechas\`.  
\- Las tareas canceladas se ocultan por defecto, pero existe filtro para verlas.  
\- Los eventos cancelados se ocultan por defecto.  
\- Calendar muestra vista \`Mes \+ Agenda\` con eventos por fecha.  
\- Calendar no muestra tareas con fecha en el MVP actual.  
\- Home muestra tareas/eventos reales de Planner y mocks para módulos sin backend.  
\- Home muestra máximo 5 tareas.  
\- Quick Actions muestra acciones reales de Planner y acciones mock para módulos no implementados.  
\- Refetch automático es obligatorio al entrar o volver a Home/Planner.  
\- Refetch automático es obligatorio después de crear, editar, completar, verificar o cancelar tareas/eventos.  
\- Pull-to-refresh queda recomendado, pero no bloquea el MVP.  
\- Realtime verdadero es deseable, pero no obligatorio.

\#\#\# Lógica de templates, responsabilidades y categorías

\- Las responsabilidades iniciales son las ya existentes en el proyecto.  
\- \`Otro\` se permite como categoría libre.  
\- Si el usuario elige una responsabilidad/template, el frontend autocompleta título/categoría.  
\- Si el usuario edita manualmente el título, el frontend no debe pisarlo automáticamente.  
\- \`category\` funciona como label visible derivado.  
\- \`template\_key\` solo se manda al backend si el backend actual lo soporta.  
\- Si el backend no soporta \`template\_key\`, se usa solo como metadata interna del frontend.

\#\#\# Lógica de tareas

\- Prioridad: \`Baja\`, \`Normal\`, \`Alta\`.  
\- Responsable visible, pero asignación opcional para no bloquear creación.  
\- Verificación visible para todas las tareas.  
\- Completar tarea debe ser una acción de 1 tap.  
\- Si la tarea no requiere verificación, completar la deja completada.  
\- Si requiere verificación, completar la deja pendiente de verificación.  
\- Verificar es una acción separada.  
\- El frontend debe mostrar errores si backend rechaza permisos o verificación propia.  
\- Vencida es estado calculado visualmente, no status persistido.

\#\#\# Lógica de eventos

\- Crear evento usa bottom sheet grande.  
\- Evento requiere título y fecha.  
\- Calendar muestra eventos asociados al día seleccionado.  
\- Cancelar evento no debe mostrarse como hard delete.  
\- Eventos cancelados se ocultan por defecto.

\#\#\# Lógica de integración backend/frontend

\- Planner frontend usa \`/api/planner\`.  
\- No usa Supabase directo.  
\- No usa legacy \`tasks\`, \`events\`, \`schedules\`.  
\- No manda \`household\_id\`.  
\- Backend resuelve \`active\_household\`.  
\- \`assigned\_to\_member\_id\` debe ser \`household\_members.id\`.  
\- Backend resuelve creador, completador y verificador.  
\- Home consume datos de Planner vía summary o services Planner.  
