...
export type PresenceSharingMode = 'off' | 'foreground' | 'background';
...
export type PresenceMember = {
  ...
  sharing_mode: PresenceSharingMode | null;
...
export type UpdatePresenceLocationPayload = {
  sharing_enabled?: boolean;
  sharing_mode?: PresenceSharingMode;
  history_enabled?: boolean;
  latitude?: number;
  longitude?: number;
  accuracy_meters?: number | null;
...
export const fetchPresencePlaces = (accessToken: string) =>
  requestJson<{ household_id: string; places: PresencePlace[] }>('/api/presence/places', {
    accessToken,
    timeoutMs: 8000,
    contextScope: 'presence:places',
  });
