...
export type RequestJsonOptions = {
  ...
  /** Development-only hook used by a narrow client contract validator. */
  onParsedResponse?: (response: ParsedJsonResponse) => void;
};
...
export async function requestJson<T>(path: string, options: RequestJsonOptions = {}): Promise<T> {
  ...
  if (onParsedResponse) {
    try {
      onParsedResponse({ status: response.status, rawBody: text, parsedBody: payload });
    } catch (caught) {
      logApiDebug('response-debug-hook-failed', {
        endpoint: path,
