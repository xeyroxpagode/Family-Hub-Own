...
export function PlannerSchedulesScreen({ refreshKey }: { refreshKey?: number }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setEditorVisible(false); await load(true);
  ...
  const remove = () => {
    ...
    Alert.alert('Eliminar horario', `Eliminar ${editing.title}?`, [{ text: 'Cancelar', style: 'cancel' }, { text: 'Eliminar', style: 'destructive', onPress: async () => {
      try { await deleteHouseholdSchedule(accessToken, editing.id); setEditorVisible(false); await load(true); }
      catch (cause) { Alert.alert('No pudimos eliminar', cause instanceof Error ? cause.message : 'Intenta nuevamente.'); }
    } }]);
  };
  const changePrivacy = async (value: boolean) => { if (!accessToken) return; setPrivacy(value); try { await updateSchedulePrivacy(accessToken, value); } catch (cause) { setPrivacy(!value); Alert.alert('No pudimos actualizar', cause instanceof Error ? cause.message : 'Intenta nuevamente.'); } };
