...
export function AccountFormSheet({
  ...
  const canSubmitEdit =
    ...
    (balanceMode === 'unknown' || canonicalInitialAmount !== null);
  ...
  const resetDraft = () => {
    ...
    setAmountText('');
    setAmount(parseMoneyInputText('', STARTING_DRAFT.currency));
  ...
  const submit = async () => {
    ...
    if (balanceMode === 'known' && canonicalInitialAmount === null) {
      setSubmitError('Revisa el monto ingresado.');
      setStep('edit');
      return;
    }
    ...
    try {
      const payload = {
        ...
        accountType,
        contextType,
        ...(balanceMode === 'known' && canonicalInitialAmount !== null
          ? {
            initialBalance: {
      ...
      <View style={styles.fieldGroup}>
        ...
        <View style={styles.balanceMode} accessibilityRole="radiogroup">
          ...
          >
            ...
            />
            <AppText variant="bodySmall" weight="800">Ingresar {balanceLabel.toLowerCase()}</AppText>
          </InteractivePressable>
        </View>
        ...
        {balanceMode === 'known' ? (
          <MoneyInput
            ...
            helperText="Representa AHORA. No incluyas signo."
          />
        ) : (
          <AppText variant="caption" tone="tertiary">
            La cuenta se crea en estado UNKNOWN. Vas a poder establecer el saldo despues, cuando lo sepas.
          </AppText>
        )}
      </View>
