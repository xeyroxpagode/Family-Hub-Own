...
const commands = {
  ...
  'experience-preferences': () => runCommand(
    'Experience Mode preferences tests',
    'node',
    ['scripts/compiled/scripts/experience_preferences_tests.js'],
  ),
  'simple-foundation': () => runCommand(
    'Simple responsive/accessibility foundation tests',
    'node',
    ['scripts/compiled/scripts/simple_foundation_tests.js'],
  ),
...
'finance-6c4-analysis-progress': () => runCommand(
  ...
  'finance-6e4-category-pool-default-frontend': () => {
    const env = {
      ...process.env,
      EXPO_PUBLIC_API_URL: process.env.EXPO_PUBLIC_API_URL || 'http://127.0.0.1:3001',
      NODE_PATH: path.join(repositoryRoot, 'tests', 'stubs'),
    ...
    return runCommand(
      'Finance 6E.4 Category Pool Default frontend tests',
      'node',
      ['scripts/compiled/scripts/finance_6e4_category_pool_default_tests.js'],
      { env },
    );
  },
  'finance-6f-spending-limits-frontend': () => {
    const env = {
      ...process.env,
      EXPO_PUBLIC_API_URL: process.env.EXPO_PUBLIC_API_URL || 'http://127.0.0.1:3001',
      NODE_PATH: path.join(repositoryRoot, 'tests', 'stubs'),
    ...
    return runCommand(
      'Finance 6F Spending Limits Frontend tests',
      'node',
      ['scripts/compiled/scripts/finance_6f_spending_limits_frontend_tests.js'],
      { env },
    );
  },
...
const suites = {
  ...
  frontend: ['compile-frontend-tests', 'core-frontend', 'g0.3', 'experience-preferences', 'simple-foundation'],
