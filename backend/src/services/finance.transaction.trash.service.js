...
async function trashFinanceTransaction(financeContext, body = {}) {
  ...
  const { data, error } = await financeContext.client.rpc('finance_trash_transaction_v1', {
  ...
  if (error) {
    ...
    if (isRlsViolation) {
      throw createHttpError(403, 'No tenes permiso para mover esta transaccion a papelera.', 'finance_transaction_trash_forbidden');
    }
